...
describe('StrengthValidator - XOR Logic', () => {
  ...
  describe('通常モード (革命なし、11バックなし)', () => {
    it('3 の上に 4 を出せる', () => {
      ...
      // 4を出す
      const cards = [card4];
      const context = { isRevolution: false, isElevenBack: false, field, suitLock: null, numberLock: false, ruleSettings: DEFAULT_RULE_SETTINGS };
  ...
  describe('革命モード (革命あり、11バックなし)', () => {
    it('4 の上に 3 を出せる', () => {
      ...
      // 3を出す（革命中なので弱い方が強い）
      const cards = [card3];
      const context = { isRevolution: true, isElevenBack: false, field, suitLock: null, numberLock: false, ruleSettings: DEFAULT_RULE_SETTINGS };
  ...
  describe('11バックモード (革命なし、11バックあり)', () => {
    it('4 の上に 3 を出せる', () => {
      ...
      const cards = [card3];
      const context = { isRevolution: false, isElevenBack: true, field, suitLock: null, numberLock: false, ruleSettings: DEFAULT_RULE_SETTINGS };
  ...
  describe('革命+11バックモード (両方あり - XORで通常に戻る)', () => {
    it('3 の上に 4 を出せる (通常と同じ)', () => {
      ...
      const cards = [card4];
      const context = { isRevolution: true, isElevenBack: true, field, suitLock: null, numberLock: false, ruleSettings: DEFAULT_RULE_SETTINGS };
